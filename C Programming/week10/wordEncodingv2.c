...
void printleaf(node_t* tree, char lastcode[]){
    if(tree == NULL){
        return;
    }
    char l[10000];
    ...
    strcat(l,"0");
    strcat(r,"1");
    printleaf(tree->left,l);
    if(tree->left == NULL && tree->right == NULL){
        printf("%s: %s\n",tree->data,lastcode);    
    }
    printleaf(tree->right,r);
}
...
int main(){
    ...
    while(heap->count != 2){
        ...
        tmp->left = tmp1;
        tmp->right = tmp2;
        strcpy(tmp->data,tmp1->data);
        strcat(tmp->data,tmp2->data);
        tmp->priority = tmp1->priority + tmp2->priority;
        heap->array[heap->count] = tmp;
        heap->count++;
